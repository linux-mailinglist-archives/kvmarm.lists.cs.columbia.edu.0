...
Received: from mm01.cs.columbia.edu (mm01.cs.columbia.edu [128.59.11.253])
	by mail.lfdr.de (Postfix) with ESMTP id 9456E87ABD
	for <lists+kvmarm@lfdr.de>; Fri,  9 Aug 2019 15:00:24 +0200 (CEST)
Received: from localhost (localhost [127.0.0.1])
	by mm01.cs.columbia.edu (Postfix) with ESMTP id 0ECDE4A575;
	Fri,  9 Aug 2019 09:00:24 -0400 (EDT)
...
Received: from mm01.cs.columbia.edu ([127.0.0.1])
	...
	with ESMTP id KRMxPeUVV7jw; Fri,  9 Aug 2019 09:00:23 -0400 (EDT)
Received: from mm01.cs.columbia.edu (localhost [127.0.0.1])
	by mm01.cs.columbia.edu (Postfix) with ESMTP id 9666C4A531;
	Fri,  9 Aug 2019 09:00:21 -0400 (EDT)
Received: from localhost (localhost [127.0.0.1])
 by mm01.cs.columbia.edu (Postfix) with ESMTP id 895DA4A4F7
 for <kvmarm@lists.cs.columbia.edu>; Fri,  9 Aug 2019 09:00:20 -0400 (EDT)
X-Virus-Scanned: at lists.cs.columbia.edu
Received: from mm01.cs.columbia.edu ([127.0.0.1])
 by localhost (mm01.cs.columbia.edu [127.0.0.1]) (amavisd-new, port 10024)
 with ESMTP id hn2vhVlfHXqu for <kvmarm@lists.cs.columbia.edu>;
 Fri,  9 Aug 2019 09:00:17 -0400 (EDT)
Received: from mx1.redhat.com (mx1.redhat.com [209.132.183.28])
 by mm01.cs.columbia.edu (Postfix) with ESMTPS id A9E1E4A4FE
 for <kvmarm@lists.cs.columbia.edu>; Fri,  9 Aug 2019 09:00:17 -0400 (EDT)
...
X-Greylist: Sender IP whitelisted, not delayed by milter-greylist-4.5.16
 (mx1.redhat.com [10.5.110.38]); Fri, 09 Aug 2019 13:00:17 +0000 (UTC)
...
> >>>> Jintack and Christoffer. Since then, the series has been substantial=
ly
...
> >>>> (we're missing many critical bits for fully emulate EL2), but the id=
ea
...
> >>>>
> >>>> For the most courageous of you, I've put out a branch[1] containing =
this
> >>>> and a bit more. Of course, you'll need some userspace. Andre maintai=
ns
...
> >>>>   KVM: arm64: nv: Reset VCPU to EL2 registers if VCPU nested virt is=
 set
...
> >>>>   KVM: arm64: nv: vgic-v3: Take cpu_if pointer directly instead of v=
cpu
...
> >>>>   KVM: arm64: nv: Trap SPSR_EL1, ELR_EL1 and VBAR_EL1 from virtual E=
L2
...
> >>>>   KVM: arm/arm64: nv: Factor out stage 2 page table data from struct=
 kvm
> >>>>   KVM: arm64: nv: Move last_vcpu_ran to be per s2 mmu
> >>>>   KVM: arm64: nv: Don't always start an S2 MMU search from the begin=
ning
...
> >>>>
> >>> When working on adding support for EL2 to kvm-unit-tests I was able t=
o trigger
> >>> the following warning:
> >>>
> >>> # ./lkvm run -f psci.flat -m 128 -c 8 --console serial --irqchip gicv=
3 --nested
...
> >>> [=A0=A0 24.384866] CPU: 3 PID: 160 Comm: kvm-vcpu-1 Tainted: G       =
 W
> >>> 5.2.0-rc5-00060-g7dbce63bd1c7 #145
...
> >>> [=A0=A0 24.394266] CPU: 3 PID: 160 Comm: kvm-vcpu-1 Tainted: G       =
 W
> >>> 5.2.0-rc5-00060-g7dbce63bd1c7 #145
...
> >>>     While you have requested a virtio-net device, the guest kernel di=
d not
> >>> initialize it.
> >>>     Please make sure that the guest kernel was compiled with CONFIG_V=
IRTIO_NET=3Dy
> >>> enabled in .config.
> >>>
> >>> [..]
> >> Did some investigating and this was caused by a bug in kvm-unit-tests =
(the fix
> >> for it will be part of the EL2 patches for kvm-unit-tests). The guest =
was trying
> >> to fetch an instruction from address 0x200, which KVM interprets as a =
prefetch
> >> abort on an I/O address and ends up calling kvm_inject_pabt. The code =
from
> >> arch/arm64/kvm/inject_fault.c doesn't know anything about nested virtu=
alization,
> >> and it sets the VCPU mode directly to PSR_MODE_EL1h. This makes_hyp_ct=
xt return
...
> >> h{p,v}timer->loaded will not be set to false. In the corresponding cal=
l to
> >> kvm_timer_vcpu_load, KVM will try to emulate the hptimer and hvtimer, =
which
> >> still have loaded =3D true. And this causes the warning I saw.
> >>
> > Hi Alexandru,
> >
> > While a unit test in kvm-unit-tests may not do what it should in order =
to
...
> > issue you could write a kvm-unit-tests test to specifically reproduce t=
he
...
> > drew
> >
> It was a host bug triggered by a bug in kvm-unit-tests. The kvm-unit-test=
s bug
> is a real bug because it goes against the intent of the psci test. It was=
n't
> discovered until now because with the upstream version of Linux we don't =
get any
> messages about it. I'll post a patch for it as soon as I can and we can d=
iscuss
