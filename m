...
Received: from mm01.cs.columbia.edu (mm01.cs.columbia.edu [128.59.11.253])
	by mail.lfdr.de (Postfix) with ESMTP id AA45A16FCBE
	for <lists+kvmarm@lfdr.de>; Wed, 26 Feb 2020 11:56:47 +0100 (CET)
Received: from localhost (localhost [127.0.0.1])
	by mm01.cs.columbia.edu (Postfix) with ESMTP id 15DA44AF74;
	Wed, 26 Feb 2020 05:56:47 -0500 (EST)
...
Received: from mm01.cs.columbia.edu ([127.0.0.1])
	...
	with ESMTP id NN-GUvRJ1Bcg; Wed, 26 Feb 2020 05:56:46 -0500 (EST)
Received: from mm01.cs.columbia.edu (localhost [127.0.0.1])
	by mm01.cs.columbia.edu (Postfix) with ESMTP id E0E144AF76;
	Wed, 26 Feb 2020 05:56:42 -0500 (EST)
Received: from localhost (localhost [127.0.0.1])
 by mm01.cs.columbia.edu (Postfix) with ESMTP id AC9CF4AF69
 for <kvmarm@lists.cs.columbia.edu>; Wed, 26 Feb 2020 05:56:41 -0500 (EST)
X-Virus-Scanned: at lists.cs.columbia.edu
Received: from mm01.cs.columbia.edu ([127.0.0.1])
 by localhost (mm01.cs.columbia.edu [127.0.0.1]) (amavisd-new, port 10024)
 with ESMTP id 1fudfnyVE2hI for <kvmarm@lists.cs.columbia.edu>;
 Wed, 26 Feb 2020 05:56:37 -0500 (EST)
Received: from mx0a-001b2d01.pphosted.com (mx0a-001b2d01.pphosted.com
 [148.163.156.1])
 by mm01.cs.columbia.edu (Postfix) with ESMTPS id 2D12D4AEFC
 for <kvmarm@lists.cs.columbia.edu>; Wed, 26 Feb 2020 05:56:37 -0500 (EST)
Received: from pps.filterd (m0098394.ppops.net [127.0.0.1])
 by mx0a-001b2d01.pphosted.com (8.16.0.42/8.16.0.42) with SMTP id
 01QArl05014121
 for <kvmarm@lists.cs.columbia.edu>; Wed, 26 Feb 2020 05:56:36 -0500
Received: from e06smtp02.uk.ibm.com (e06smtp02.uk.ibm.com [195.75.94.98])
 by mx0a-001b2d01.pphosted.com with ESMTP id 2ydcp4j9s8-1
 (version=TLSv1.2 cipher=AES256-GCM-SHA384 bits=256 verify=NOT)
 for <kvmarm@lists.cs.columbia.edu>; Wed, 26 Feb 2020 05:56:35 -0500
Received: from localhost
 by e06smtp02.uk.ibm.com with IBM ESMTP SMTP Gateway: Authorized Use Only!
 Violators will be prosecuted
 for <kvmarm@lists.cs.columbia.edu> from <rppt@kernel.org>;
...
> Le 26/02/2020 =E0 10:13, Mike Rapoport a =E9crit=A0:
...
> > > > > Implement primitives necessary for the 4th level folding, add wal=
ks of p4d
...
> https://patchwork.ozlabs.org/project/linuxppc-dev/list/?series=3D152102
> =
...
> > diff --git a/arch/powerpc/include/asm/book3s/32/pgtable.h b/arch/powerp=
c/include/asm/book3s/32/pgtable.h
...
> > diff --git a/arch/powerpc/include/asm/book3s/64/pgalloc.h b/arch/powerp=
c/include/asm/book3s/64/pgalloc.h
> > index a41e91bd0580..69c5b051734f 100644
> > --- a/arch/powerpc/include/asm/book3s/64/pgalloc.h
> > +++ b/arch/powerpc/include/asm/book3s/64/pgalloc.h
> > @@ -85,9 +85,9 @@ static inline void pgd_free(struct mm_struct *mm, pgd=
_t *pgd)
> >   	kmem_cache_free(PGT_CACHE(PGD_INDEX_SIZE), pgd);
> >   }
> > -static inline void pgd_populate(struct mm_struct *mm, pgd_t *pgd, pud_=
t *pud)
> > +static inline void p4d_populate(struct mm_struct *mm, p4d_t *pgd, pud_=
t *pud)
> >   {
> > -	*pgd =3D  __pgd(__pgtable_ptr_val(pud) | PGD_VAL_BITS);
> > +	*pgd =3D  __p4d(__pgtable_ptr_val(pud) | PGD_VAL_BITS);
> >   }
> >   static inline pud_t *pud_alloc_one(struct mm_struct *mm, unsigned lon=
g addr)
...
PGD - 1))
> >   #define pud_index(address) (((address) >> (PUD_SHIFT)) & (PTRS_PER_PU=
D - 1))
...
> >   }
> >   #endif /* __ASSEMBLY__ */
> > diff --git a/arch/powerpc/include/asm/book3s/64/radix.h b/arch/powerpc/=
...
> > diff --git a/arch/powerpc/include/asm/nohash/32/pgtable.h b/arch/powerp=
c/include/asm/nohash/32/pgtable.h
...
> >   #include <asm-generic/pgtable-nopmd.h>
> >   #ifndef __ASSEMBLY__
> > diff --git a/arch/powerpc/include/asm/nohash/64/pgalloc.h b/arch/powerp=
c/include/asm/nohash/64/pgalloc.h
...
g addr)
> >   {
> > diff --git a/arch/powerpc/include/asm/nohash/64/pgtable-4k.h b/arch/pow=
erpc/include/asm/nohash/64/pgtable-4k.h
...
> > +#include <asm-generic/pgtable-nop4d.h>
> >   /*
> >    * Entries per page directory level.  The PTE level must use a 64b re=
cord
> > @@ -45,41 +45,41 @@
...
> >   #define pud_ERROR(e) \
> > diff --git a/arch/powerpc/include/asm/nohash/64/pgtable.h b/arch/powerp=
c/include/asm/nohash/64/pgtable.h
...
> > @@ -499,13 +499,14 @@ void kvmppc_free_pgtable_radix(struct kvm *kvm, p=
gd_t *pgd, unsigned int lpid)
> >   	unsigned long ig;
...
> >   	}
> >   }
> > @@ -566,6 +567,7 @@ int kvmppc_create_pte(struct kvm *kvm, pgd_t *pgtab=
le, pte_t pte,
...
> > @@ -573,9 +575,11 @@ int kvmppc_create_pte(struct kvm *kvm, pgd_t *pgta=
ble, pte_t pte,
...
> > @@ -596,13 +600,13 @@ int kvmppc_create_pte(struct kvm *kvm, pgd_t *pgt=
able, pte_t pte,
...
> > @@ -1220,6 +1224,7 @@ static ssize_t debugfs_radix_read(struct file *fi=
le, char __user *buf,
> >   	pgd_t *pgt;
...
> >   	pmd_t pmd, *pmdp;
> >   	pte_t *ptep;
> > @@ -1292,13 +1297,14 @@ static ssize_t debugfs_radix_read(struct file *=
...
> > @@ -64,17 +64,19 @@ static int early_map_kernel_page(unsigned long ea, =
unsigned long pa,
> >   {
...
> >   		ptep =3D (pte_t *)pudp;
> >   		goto set_the_pte;
> > @@ -114,6 +116,7 @@ static int __map_kernel_page(unsigned long ea, unsi=
gned long pa,
> >   {
> >   	unsigned long pfn =3D pa >> PAGE_SHIFT;
> >   	pgd_t *pgdp;
...
> >   	pmd_t *pmdp;
> >   	pte_t *ptep;
> > @@ -136,7 +139,8 @@ static int __map_kernel_page(unsigned long ea, unsi=
gned long pa,
> >   	 * boot.
> >   	 */
> >   	pgdp =3D pgd_offset_k(ea);
...
> >   		return -ENOMEM;
> >   	if (map_page_size =3D=3D PUD_SIZE) {
> > @@ -173,6 +177,7 @@ void radix__change_memory_range(unsigned long start=
, unsigned long end,
> >   {
> >   	unsigned long idx;
> >   	pgd_t *pgdp;
...
> >   	pmd_t *pmdp;
> >   	pte_t *ptep;
> > @@ -185,7 +190,8 @@ void radix__change_memory_range(unsigned long start=
, unsigned long end,
...
> >   			continue;
> >   		if (pud_is_leaf(*pudp)) {
> > @@ -847,6 +853,7 @@ static void __meminit remove_pagetable(unsigned lon=
...
> > @@ -854,15 +861,16 @@ static void __meminit remove_pagetable(unsigned l=
...
> >   	}
> > diff --git a/arch/powerpc/mm/book3s64/subpage_prot.c b/arch/powerpc/mm/=
book3s64/subpage_prot.c
> > index 2ef24a53f4c9..25a0c044bd93 100644
> > --- a/arch/powerpc/mm/book3s64/subpage_prot.c
> > +++ b/arch/powerpc/mm/book3s64/subpage_prot.c
> > @@ -54,15 +54,17 @@ static void hpte_flush_range(struct mm_struct *mm, =
unsigned long addr,
> >   			     int npages)
> >   {
> >   	pgd_t *pgd;
...
> > @@ -119,6 +119,7 @@ static int __hugepte_alloc(struct mm_struct *mm, hu=
gepd_t *hpdp,
> >   pte_t *huge_pte_alloc(struct mm_struct *mm, unsigned long addr, unsig=
ned long sz)
> >   {
> >   	pgd_t *pg;
> > +	p4d_t *p4;
...
> > @@ -128,20 +129,21 @@ pte_t *huge_pte_alloc(struct mm_struct *mm, unsig=
ned long addr, unsigned long sz
> >   	addr &=3D ~(sz-1);
...
> > @@ -166,10 +168,10 @@ pte_t *huge_pte_alloc(struct mm_struct *mm, unsig=
ned long addr, unsigned long sz
> >   #else
...
> > @@ -390,7 +392,7 @@ static void hugetlb_free_pmd_range(struct mmu_gathe=
r *tlb, pud_t *pud,
...
> >   {
> > @@ -400,7 +402,7 @@ static void hugetlb_free_pud_range(struct mmu_gathe=
r *tlb, pgd_t *pgd,
> >   	start =3D addr;
> >   	do {
> > -		pud =3D pud_offset(pgd, addr);
...
> > @@ -435,8 +437,8 @@ static void hugetlb_free_pud_range(struct mmu_gathe=
r *tlb, pgd_t *pgd,
> >   	if (end - 1 > ceiling - 1)
> >   		return;
...
> >   	unsigned long next;
> >   	/*
> > @@ -471,10 +474,11 @@ void hugetlb_free_pgd_range(struct mmu_gather *tl=
b,
> >   	do {
> >   		next =3D pgd_addr_end(addr, end);
...
> > @@ -36,7 +36,7 @@ static int __init kasan_init_shadow_page_tables(unsig=
ned long k_start, unsigned
> >   	unsigned long k_cur, k_next;
> >   	pte_t *new =3D NULL;
> > -	pmd =3D pmd_offset(pud_offset(pgd_offset_k(k_start), k_start), k_star=
t);
> > +	pmd =3D pmd_offset(pud_offset(p4d_offset(pgd_offset_k(k_start), k_sta=
rt), k_start), k_start);
> >   	for (k_cur =3D k_start; k_cur !=3D k_end; k_cur =3D k_next, pmd++) {
> >   		k_next =3D pgd_addr_end(k_cur, k_end);
> > @@ -78,7 +78,7 @@ static int __init kasan_init_region(void *start, size=
_t size)
> >   	block =3D memblock_alloc(k_end - k_start, PAGE_SIZE);
> >   	for (k_cur =3D k_start & PAGE_MASK; k_cur < k_end; k_cur +=3D PAGE_S=
IZE) {
> > -		pmd_t *pmd =3D pmd_offset(pud_offset(pgd_offset_k(k_cur), k_cur), k_=
cur);
> > +		pmd_t *pmd =3D pmd_offset(pud_offset(p4d_offset(pgd_offset_k(k_cur),=
 k_cur), k_cur), k_cur);
...
> > +	pmd_t *pmd =3D pmd_offset(pud_offset(p4d_offset(pgd_offset_k(addr), a=
ddr), addr), addr);
...
> > -			vaddr), vaddr), vaddr);
> > +	return pte_offset_kernel(pmd_offset(pud_offset(p4d_offset(pgd_offset_=
k(vaddr),
...
> > @@ -104,7 +104,7 @@ unsigned long __init mmu_mapin_ram(unsigned long ba=
se, unsigned long top)
> >   		pmd_t *pmdp;
> >   		unsigned long val =3D p | _PMD_SIZE_16M | _PAGE_EXEC | _PAGE_HWWRIT=
E;
...
> >   		*pmdp++ =3D __pmd(val);
> >   		*pmdp++ =3D __pmd(val);
> > @@ -119,7 +119,7 @@ unsigned long __init mmu_mapin_ram(unsigned long ba=
se, unsigned long top)
> >   		pmd_t *pmdp;
...
> > diff --git a/arch/powerpc/mm/nohash/book3e_pgtable.c b/arch/powerpc/mm/=
nohash/book3e_pgtable.c
> > index 4637fdd469cf..77884e24281d 100644
> > --- a/arch/powerpc/mm/nohash/book3e_pgtable.c
> > +++ b/arch/powerpc/mm/nohash/book3e_pgtable.c
> > @@ -73,6 +73,7 @@ static void __init *early_alloc_pgtable(unsigned long=
 size)
> >   int __ref map_kernel_page(unsigned long ea, unsigned long pa, pgprot_=
t prot)
> >   {
> >   	pgd_t *pgdp;
> > +	p4d_t *p4dp;
...
> >   	pmd_t *pmdp;
> >   	pte_t *ptep;
> > @@ -80,7 +81,8 @@ int __ref map_kernel_page(unsigned long ea, unsigned =
long pa, pgprot_t prot)
...
> >   			return -ENOMEM;
> >   		pmdp =3D pmd_alloc(&init_mm, pudp, ea);
> > @@ -91,13 +93,12 @@ int __ref map_kernel_page(unsigned long ea, unsigne=
d long pa, pgprot_t prot)
> >   			return -ENOMEM;
> >   	} else {
> >   		pgdp =3D pgd_offset_k(ea);
...
> > @@ -265,6 +265,7 @@ int huge_ptep_set_access_flags(struct vm_area_struc=
t *vma,
...
> >   	pud_t *pud;
> >   	pmd_t *pmd;
> > @@ -272,7 +273,9 @@ void assert_pte_locked(struct mm_struct *mm, unsign=
ed long addr)
> >   		return;
...
> >   	if (hpage_shift)
> >   		*hpage_shift =3D 0;
> > @@ -325,24 +329,28 @@ pte_t *__find_linux_pte(pgd_t *pgdir, unsigned lo=
ng ea,
> >   	if (is_thp)
> >   		*is_thp =3D false;
...
> >   		goto out_huge;
> >   	}
> > @@ -352,7 +360,7 @@ pte_t *__find_linux_pte(pgd_t *pgdir, unsigned long=
 ea,
> >   	 * irq disabled
> >   	 */
> >   	pdshift =3D PUD_SHIFT;
...
> > @@ -63,7 +63,7 @@ int __ref map_kernel_page(unsigned long va, phys_addr=
_t pa, pgprot_t prot)
> >   	int err =3D -ENOMEM;
> >   	/* Use upper 10 bits of VA to index the first level map */
> > -	pd =3D pmd_offset(pud_offset(pgd_offset_k(va), va), va);
> > +	pd =3D pmd_offset(pud_offset(p4d_offset(pgd_offset_k(va), va), va), v=
a);
...
> > - */
> > -static int
> > -get_pteptr(struct mm_struct *mm, unsigned long addr, pte_t **ptep, pmd=
_t **pmdp)
> > -{
> > -        pgd_t	*pgd;
> > -	pud_t	*pud;
...
> > -}
> > -
> >   static int __change_page_attr_noflush(struct page *page, pgprot_t pro=
t)
> >   {
> >   	pte_t *kpte;
> >   	pmd_t *kpmd;
...
> > +	va =3D address & PAGE_MASK;
> >   	if (v_block_mapped(address))
> >   		return 0;
> > -	if (!get_pteptr(&init_mm, address, &kpte, &kpmd))
> > +
> > +	kpmd =3D pmd_offset(pud_offset(p4d_offset(pgd_offset_k(va), va), va),=
 va);
> > +	if (!pmd_present(*kpmd))
> >   		return -EINVAL;
> > +
> > +	kpte =3D pte_offset_map(kpmd, va);
...
> >   }
> >   #endif
> > diff --git a/arch/powerpc/mm/ptdump/hashpagetable.c b/arch/powerpc/mm/p=
tdump/hashpagetable.c
> > index a07278027c6f..ac360ad865a8 100644
> > --- a/arch/powerpc/mm/ptdump/hashpagetable.c
> > +++ b/arch/powerpc/mm/ptdump/hashpagetable.c
> > @@ -417,9 +417,9 @@ static void walk_pmd(struct pg_state *st, pud_t *pu=
...
> >   	unsigned long addr;
> >   	unsigned int i;
> > @@ -431,6 +431,20 @@ static void walk_pud(struct pg_state *st, pgd_t *p=
