...
Received: from mm01.cs.columbia.edu (mm01.cs.columbia.edu [128.59.11.253])
	by mail.lfdr.de (Postfix) with ESMTP id 9375046218C
	for <lists+kvmarm@lfdr.de>; Mon, 29 Nov 2021 21:06:58 +0100 (CET)
Received: from localhost (localhost [127.0.0.1])
	by mm01.cs.columbia.edu (Postfix) with ESMTP id 440E2407F1;
	Mon, 29 Nov 2021 15:06:58 -0500 (EST)
...
Received: from mm01.cs.columbia.edu ([127.0.0.1])
	...
	with ESMTP id 9kTumKOV6tQh; Mon, 29 Nov 2021 15:06:55 -0500 (EST)
Received: from mm01.cs.columbia.edu (localhost [127.0.0.1])
	by mm01.cs.columbia.edu (Postfix) with ESMTP id A4F1C4B1C1;
	Mon, 29 Nov 2021 15:06:55 -0500 (EST)
Received: from localhost (localhost [127.0.0.1])
 by mm01.cs.columbia.edu (Postfix) with ESMTP id B660F4B0F5
 for <kvmarm@lists.cs.columbia.edu>; Mon, 29 Nov 2021 15:06:54 -0500 (EST)
X-Virus-Scanned: at lists.cs.columbia.edu
Received: from mm01.cs.columbia.edu ([127.0.0.1])
 by localhost (mm01.cs.columbia.edu [127.0.0.1]) (amavisd-new, port 10024)
 with ESMTP id Pma368wsMiSD for <kvmarm@lists.cs.columbia.edu>;
 Mon, 29 Nov 2021 15:06:51 -0500 (EST)
Received: from sin.source.kernel.org (sin.source.kernel.org [145.40.73.55])
 by mm01.cs.columbia.edu (Postfix) with ESMTPS id 79FCE4B1A3
 for <kvmarm@lists.cs.columbia.edu>; Mon, 29 Nov 2021 15:06:50 -0500 (EST)
Received: from smtp.kernel.org (relay.kernel.org [52.25.139.140])
 (using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
 (No client certificate requested)
 by sin.source.kernel.org (Postfix) with ESMTPS id 36F16CE13D8;
 Mon, 29 Nov 2021 20:06:48 +0000 (UTC)
Received: by smtp.kernel.org (Postfix) with ESMTPSA id 25942C53FCF;
 Mon, 29 Nov 2021 20:06:46 +0000 (UTC)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=kernel.org;
 s=k20201202; t=1638216406;
 bh=v8PNXU8eqe6gtDxTIrJTfRk0WR0eAamzcjambcxVTW0=;
 h=From:To:Cc:Subject:Date:In-Reply-To:References:From;
 b=
Received: from sofa.misterjones.org ([185.219.108.64] helo=why.lan)
 by disco-boy.misterjones.org with esmtpsa (TLS1.3) tls
 TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384 (Exim 4.94.2)
 (envelope-from <maz@kernel.org>)
 id 1mrmr2-008gvR-LX; Mon, 29 Nov 2021 20:02:24 +0000
From: Marc Zyngier <maz@kernel.org>
To: linux-arm-kernel@lists.infradead.org, kvmarm@lists.cs.columbia.edu,
 kvm@vger.kernel.org
Subject: [PATCH v5 45/69] KVM: arm64: nv: Trap and emulate AT instructions
 from virtual EL2
Date: Mon, 29 Nov 2021 20:01:26 +0000
Message-Id: <20211129200150.351436-46-maz@kernel.org>
...
index 12f4bc8a805b..b603466803d2 100644
...
index 50d5e4de244c..abea7e164400 100644
--- a/arch/arm64/include/asm/kvm_asm.h
+++ b/arch/arm64/include/asm/kvm_asm.h
@@ -207,6 +207,8 @@ extern void __kvm_tlb_flush_vmid_ipa(struct kvm_s2_mmu *mmu, phys_addr_t ipa,
...
index c77fe5401826..b673adeb190f 100644
...
index 923ee13728b4..8e19da1a07e1 100644
...
index da80c969e623..657a74bab73c 100644
--- a/arch/arm64/kvm/hyp/vhe/switch.c
+++ b/arch/arm64/kvm/hyp/vhe/switch.c
@@ -42,9 +42,10 @@ static void __activate_traps(struct kvm_vcpu *vcpu)
 		if (!vcpu_el2_e2h_is_set(vcpu)) {
 			/*
...
@@ -69,6 +70,14 @@ static void __activate_traps(struct kvm_vcpu *vcpu)
 			hcr &= ~HCR_TVM;
...
index 222f8c6f1d7e..aa4d4e187c9f 100644
--- a/arch/arm64/kvm/sys_regs.c
+++ b/arch/arm64/kvm/sys_regs.c
@@ -1703,6 +1703,10 @@ static bool access_sp_el1(struct kvm_vcpu *vcpu,
 	return true;
 }
...
@@ -2236,12 +2240,205 @@ static const struct sys_reg_desc sys_reg_descs[] = {
